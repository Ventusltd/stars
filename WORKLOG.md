...
- 2026-09-14 12:19 — The Classification star now names its five classes (Structure, Movement, Computation, Flow, Data) on the dashboard and the website; its report is `reports/CLASSIFICATION.md`. Graph ids are unchanged, so the dashboard updates the title on its next hourly check.
- 2026-09-14 12:30 — Public names settled: the homepage nest under Grid Engine is 'Relational mapping'; its entries and the dashboard cards carry one-word names (Applications, Assessment, Audit, Classification, Compatibility, Engine, Inventory, Lineage, Repositories, Reuse, Sampling, Structure); the whole is named globalgrid2050 architecture development. Graph ids unchanged; the dashboard takes the new titles at its next hourly check.
- 2026-09-14 12:40 — Decisions: judgement now has permanent keys. `decisions/dNNN.json` records (`decisions/SCHEMA.md`), made by clicks from the Decision issue form (`.github/ISSUE_TEMPLATE/decision.yml`) and filed by the `Decisions` workflow with the next number, never reused. Seeded d001 to d016, all open: the twelve unsettled constants and the four waiting decisions. `modular-star/decisions.mjs` draws them as the Decisions map (`decisions/graph.json`, registered as `decisions`), each record wired to the `block:`/`family:` id it concerns. A decided record is permission; an open record is a question. First decision to take: d014 (does geo-core settle Ek).
- 2026-09-14 12:42 — Non-coder's journey audited (`JOURNEY.md`): `start.html` added (three plain questions to the right map, subject or page); `table.html` and `code.html` gain one-tap navigation (Map, Home, Start here), plain wording, live page first, `?category=` handled, the 144 automatic blocks folded, lists ordered, a landing card on the code report; `tools/pages-check.mjs` runs in the Modular star workflow after the graph checks (own files served, no script from another host).
- 2026-09-14 12:56 — Relationship intelligence audited: 25 edge-type strings across six graphs reduced to one vocabulary of 18 (`relationships/VOCABULARY.md`; six classes: evidence, dependency, containment, debt, warning, question). `build.mjs`, `blocks.mjs` and `sense.mjs` now emit it (the Periodic table's category wire runs category → block as `contains`; the Assessment graph keeps proven, seen-together, unstable and fails apart instead of one name). `modular-star/relate.mjs` writes a relationship line on every focused card, generated from the graph and the Decisions map by shared id, idempotent; `modular-star/why.mjs` writes one traced sentence on every red card and `relationships/WHY.md` (3 red cards, 3 traced). Both run in the Modular star workflow after the graphs are built. `relationships/ENGINE-PATCH.md` is the additive dashboard change that would colour wires by class; not applied, owner's decision.
- 2026-09-14 13:01 — Coding engine hand-off live in code-generator (5211a89, run 34846587798 success; served assembly page SHA-256 matches 857ea81): every generated app gets an assembly page (apps/<name>/index.html) that loads its files in dependency order and shows what threw; needs are sorted by rules.mjs (browser, node, library, another block, declared, word, absent, missing); what is missing and every untested pair become requests/<name>-<n>.json and GitHub issues labelled work-order, raised once; the picker shows "Built from existing code" and "To be written". First orders #1–#5 for substation-finder. Applications graph family labels fixed (no "undefined" labels served). Finding for this repository: of 78 needs listed for Si, Vn, Ug, Ps, Dt, only 1 (maplibregl) is genuinely missing; 25 are browser built-ins, 15 declared inside the files, 24 words from comments and strings, 8 absent, 2 in other blocks.
